    ...
    for (let i = 0; i < countryData.length; i++) {
        ...
        appendDetails.innerHTML +=
            ...
            <div class="card mt-3" style="width:20rem">
                 ...
                 <h5 class="card-title text-center" ><b>${countryData[i].name.common.toUpperCase()}</b></h6>
                 <img class="title-image mt-2" style="width:10rem" src="${countryData[i].flags.png}" alt="Flag of ${countryData[i].name.common}">
                 </div>
                 <div class="card-body text-center">
                 <p class="body-text" mt-2><b>Capital : ${countryData[i].capital}</b></p>
                 <p class="body-text"><b>Region : ${countryData[i].region}</b></p>
                 <p class="body-text"><b>Country Code : ${countryData[i].altSpellings[0]}</b></p>
                 <p class="body-text" id="weatherApp_${i}"><b></b></p>
                 </div>
                 <div class="card-footer text-center mt-3">
                 <button type="button" class="card-btn rounded mt-2 align-center " onclick="weatherUpdate('${country}', ${i})">Click For Weather Updates</button>
                 </div></div></div></div>
            `;
    }
}
